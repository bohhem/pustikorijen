            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 auto-rows-fr">
              {branches.map((branch) => (
                <Link
                  ...
                  className="bg-white rounded-lg shadow hover:shadow-lg transition-shadow p-4 sm:p-6 flex flex-col h-full"
                >
                  <div className="flex items-start justify-between mb-3 sm:mb-4 gap-2">
                    <div className="flex-1 min-w-0">
                      <h3 className="text-lg sm:text-xl font-bold text-gray-900 break-words line-clamp-2">{branch.surname}</h3>
                      <p className="text-xs sm:text-sm text-gray-600 break-words">{formatBranchLocation(branch)}</p>
                    </div>
                    {branch.isVerified && (
                      <span className="inline-flex items-center px-2 py-1 rounded text-xs font-medium bg-green-100 text-green-800 flex-shrink-0 self-start">
                  ...
                  <div className="text-sm text-gray-700 mb-3 sm:mb-4">
                    <p className="font-mono text-xs text-gray-500 mb-2 break-all">{branch.id}</p>
                    {branch.description && (
                      <p className="line-clamp-3 text-xs sm:text-sm">{branch.description}</p>
                    )}
                  </div>
                  ...
                  <div className="mt-auto">
                      ...
                      <div className="text-xs text-gray-600 mb-3 pb-3 border-b border-gray-100">
                        <div className="flex items-start gap-1 mb-1">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <span className="break-words">{branch.foundedBy.fullName}</span>
                          </div>
                        </div>
                        <div className="flex items-start gap-1">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <span className="break-words">{formatLastActive(branch.foundedBy.lastLogin)}</span>
                          </div>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="grid grid-cols-3 gap-2 sm:gap-4 text-xs sm:text-sm text-gray-600 border-t pt-3 sm:pt-4">
                      <div className="text-center">
                        ...
                        <div className="text-xs truncate">{t('branches.members').toLowerCase()}</div>
                      </div>
                      <div className="text-center">
                        <div className="font-semibold text-sm sm:text-base">{branch.totalPeople}</div>
                        <div className="text-xs truncate">{t('branches.totalPeople').toLowerCase()}</div>
                      </div>
                      <div className="text-center">
                        <div className="font-semibold text-sm sm:text-base">{branch.totalGenerations}</div>
                        <div className="text-xs truncate">{t('branches.totalGenerations').toLowerCase()}</div>
                      </div>
                    </div>
                  </div>
                </Link>
